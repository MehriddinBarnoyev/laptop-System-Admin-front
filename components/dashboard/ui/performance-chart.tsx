...
export default function PerformanceChart() {
  ...
  // Generate chart data only on the client side after component mounts
  useEffect(() => {
    const cpuData = Array.from({ length: 24 }, () => Math.floor(Math.random() * 60) + 20)
    const memoryData = Array.from({ length: 24 }, () => Math.floor(Math.random() * 40) + 40)
    const networkData = Array.from({ length: 24 }, () => Math.floor(Math.random() * 30) + 30)
        ...
        {Array.from({ length: 24 }).map((_, i) => (
          <div key={i} className="flex space-x-0.5">
            <div
              ...
              style={{ height: chartData.cpu[i] ? `${chartData.cpu[i]}%` : "0%" }}
            ></div>
            <div
              className="w-1 bg-gradient-to-t from-purple-500 to-purple-400 rounded-t-sm"
              style={{ height: chartData.memory[i] ? `${chartData.memory[i]}%` : "0%" }}
            ></div>
            <div
              className="w-1 bg-gradient-to-t from-blue-500 to-blue-400 rounded-t-sm"
              style={{ height: chartData.network[i] ? `${chartData.network[i]}%` : "0%" }}
            ></div>
          </div>
        ))}
      </div>
